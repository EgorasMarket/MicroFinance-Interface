...
const payLoan = async (loanID, signer) => {
  try {
    const instance = contractInstance(signer);
...
const activateLoan = async (loanID, signer) => {
  ...
  const creatLoan = async ( amount, title, length, image_url, signer) =>{
    
    try {
      const instance = contractInstance(signer);
      let result = await instance.applyForLoan(amount, title, length, image_url);
      return {
        message: result.hash,
      status: true,
      }
